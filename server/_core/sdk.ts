...
class SDKServer {
  ...
  async createSessionToken(
    ...
    if (user) await db.createActiveSession(user.id, jti);
    return token;
  }
  ...
  async verifySession(
  ...
  ): Promise<{ openId: string; appId: string; name: string; sessionVersion?: number; jti?: string } | null> {
    if (!cookieValue) {
    ...
    try {
      ...
      const { openId, appId, name, sessionVersion, jti } = payload as Record<string, unknown>;
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (session.jti) {
      const active = await db.isActiveSession(session.jti);
      if (active === false) throw ForbiddenError("Session revoked; please sign in again");
      await db.touchActiveSession(session.jti);
    }
